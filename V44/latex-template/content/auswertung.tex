...
\begin{figure}[h!]
  ...
  \label{fig:zscan}
\end{figure}
Damit ergibt sich die Strahlbreite zu:
\begin{equation}
...
\begin{figure}[h!]
  ...
  \label{fig:rockscan}
\end{figure}
Die beiden abgelesenen Geometriewinkel sowie der gemittelte Wert lauten:
\begin{align}
...
\begin{equation}
  \alpha_\mathrm{g,Theorie} = \SI{0.57}{\degree}
\end{equation}
...
\begin{figure}[h!]
  ...
  \label{fig:messungr}
\end{figure}
Da die Messdaten im Bereich $\alpha_e > \SI{1.7}{\degree}$ nicht aussagekräftig sind werden diese in der Grafik nicht angezeigt. Die Reflektivität $R$ ergibt sich mit einer Abtastrate von 5 aus folgender Formel:
\begin{equation}
...
dies in der Grafik kaum auf. Die Reflektivität wird mit einem Geometriefaktor korrigiert, dieser ergibt sich mit den aus der Justage ermittelten Größen nach Gleichung (\ref{eqn:Geometriefaktorl}). Außerdem ist Abbildung (\ref{fig:messungr}) zu Vergleichszwecken die Fresenelreflektivität einer ideal glatten Siliziumoberfläche  aufgetragen.
...
\end{equation}
\subsubsection{Parrat-Algorithmus}
